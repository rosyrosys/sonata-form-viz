...
the analytical content the tool is *designed to reveal*. Figure 1
shows the recapitulation P region in the tool: the F-major key
signature (one flat) is visible at the start of the recapitulation,
and the analysis pane labels the section in real time. Three
...
any of them. Figure 2 shows the system at startup, with the timeline,
analysis pane, and OSMD-rendered score laid out together. The
remainder of this section describes the three representations (§4.1),
the color scheme (§4.2), the analysis data (§4.3), the listening
affordances (§4.4), and one design choice that deserves particular
attention — how the tool handles performer-elected exposition
repeats (§4.5).
...
![**Figure 3.** Active-listening control panel and demo-mode fallback. The yellow *Demo mode* bar is automatically shown when no audio file is available (e.g., when the copyrighted reference recording is not bundled). Below it: section-jump buttons (Exposition / Development / Recapitulation), playback-speed slider, color/cursor/loop toggles, calibration controls, and a keyboard-shortcut legend.](figures/fig_a2_timeline.png)
