...
제공한다. 본 연구는 추가로 **제시부 반복 폴딩 (exposition-repeat
folding) 시간 맵** 을 기여한다. 이는 연주자가 선택적으로 수행하는 제시부
반복 하에서도 분석적 단조성을 보존하는 piecewise 비단조 시간 함수로,
저자들이 알기로 어떤 기존 오픈 음악 시각화 시스템도 이 사례를 명시적
시간 맵으로 처리하지 않는다. 음원 파일이 없는 환경에서는 WebAudio
메트로놈으로 다운비트 클릭을 합성하여 형식 흐름만 단독으로 학습할 수
있는 *데모 모드* 를 함께 제공한다.

본 논문은 시스템 설계와 구현, 제시부 반복 폴딩 기법, 그리고 K. 545
1악장의 *재현부 하속조 시작* 이라는 비전형 사례를 다중 표상으로
시각화하는 교수법적 의의를 다룬다. 공개되는 JSON 스키마는 Allegraud
et al. (2019) 의 Algomus 주석 컨벤션과 *상호운용 가능하도록 설계* 되어,
인간 큐레이팅 분석과 분류기 출력 모두 동일한 입력 경로를 통해 도구에
적재될 수 있다. 본 도구는 MIT 라이선스 오픈소스로, 분석 스키마는 CC
BY 4.0 으로 공개된다. 학습 효과의 정량적 검증은 후속 RCT 연구로 남긴다.
...
open music-visualization system addresses explicitly. A WebAudio-based
metronome demo mode allows the formal visualization to function even
without an audio file.

This paper focuses on system design, implementation, the
exposition-repeat folding technique, and the pedagogical significance
of visualizing K. 545's atypical *subdominant recapitulation* through
multi-representational synchronization. The released JSON schema is
*intentionally interoperable* with the Algomus annotation conventions
of Allegraud et al. (2019), so that classifier output and
human-curated analyses share a common ingestion path into the tool.
The system is released as MIT-licensed open source and the analysis
schema is licensed CC BY 4.0. Empirical validation of learning
