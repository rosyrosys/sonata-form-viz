...
3. **제시부 반복 폴딩 시간 맵**: 연주자 선택 제시부 반복 하에서
   ...
   공식화). 저자가 알기로 기존 어떤 오픈 음악 시각화 시스템
...
인접 시스템들은 본 연구와 개별 기능을 공유하지만 그 조합은 공유하지
않는다. Sonic Visualiser (Cannam et al. 2010) 와 iAnalyse (Couprie
2008) 는 자유 형식 주석을 지원하지만 소나타 형식 교수를 위한 학습자
친화 도구는 아니며, Verovio Humdrum Viewer (Sapp 2017) 는 악보를
렌더링하지만 섹션 항해를 위해 음원과 동기화하지 않으며, Soundslice
와 MuseScore Web 같은 상용 악보 추종 도구는 음원과 악보를 동기화
하지만 위계적 형식 라벨을 노출하지도, 소스를 공개하지도 않는다. 따라서
본 연구의 신규성 주장은 "최초의 오픈소스 소나타 형식 시각화" 보다
좁다: "마디 도장 위계 형식 색상, 악보-음원 동기화, 그리고 분리 가능한
*데이터로서의 분석* 스키마를 결합한 최초의 오픈소스 학습자 친화 도구"
이다.
...
악보 위에서, 재생 속도로, 일어나는 모습으로 보는 경험* 이, 저자의
...
> The analytical claims above are not generated by the tool, they
> are the analytical content the tool is *designed to reveal*.
> Figure 1 shows the recapitulation P region in the tool: the
> F-major key signature (one flat) is visible at the start of the
> recapitulation, and the analysis pane labels the section in real
> time. Three features carry the pedagogical work:

위의 분석적 주장들은 도구가 *생성* 하는 것이 아니라, 도구가 *드러내도록
설계된* 분석적 내용이다. Figure 1 은 도구에서의 재현부 P 구간을
보여준다: 재현부 시작점에 바장조 조표 (단조표 하나) 가 보이고, 분석
패널이 실시간으로 섹션 라벨을 표시한다. 세 가지 기능이 교수법적 작업을
수행한다:

![**Figure 1.** Recapitulation P region (mm. 42–58) on the score with the section overlay visible at the boundary. The F-major key signature (one flat) is the visible cue that distinguishes the recapitulation P from the C-major exposition P, illustrating K. 545's subdominant-recapitulation deformation discussed in §3.3.](figures/fig_a3_recap_p.png)
...
> external dependency (OpenSheetMusicDisplay) and no build step.
> The source code, analysis data, and audio-policy documentation
> are available through the project's GitHub repository and Zenodo
> archive (§9 *Reproducibility*).

본 시스템은 정적 클라이언트-사이드 웹 애플리케이션으로, 단일 외부
의존성 (OpenSheetMusicDisplay) 외에는 빌드 단계가 없다. 소스 코드·
분석 데이터·음원 정책 문서는 프로젝트의 GitHub 저장소와 Zenodo
아카이브에서 이용 가능하다 (§9 *재현 가능성*).

> What makes the system useful for teaching is not any single
> representation but the *coordination* among them: a single
> musical present moment is rendered three ways, and the user can
> intervene in any of them. Figure 2 shows the system at startup,
> with the timeline, analysis pane, and OSMD-rendered score laid
> out together. The remainder of this section describes the three
> representations (§4.1), the color scheme (§4.2), the analysis
> data (§4.3), the listening affordances (§4.4), and one design
> choice that deserves particular attention: how the tool handles
> performer-elected exposition repeats (§4.5).

본 시스템이 교육적으로 유용한 이유는 어떤 하나의 표상이 아니라 그
표상들 사이의 *동기화* 에 있다: 단일한 음악적 현재 순간이 세 가지
방식으로 렌더되며, 사용자는 그중 어느 것에서든 개입할 수 있다.
Figure 2 는 시스템 시작 화면의 전체 모습 — 타임라인, 분석 패널,
OSMD 렌더링 악보가 함께 배치된 모습 — 을 보여준다. 이 섹션의
나머지는 세 가지 표상 (§4.1), 색상 도식 (§4.2), 분석 데이터 (§4.3),
청취 어포던스 (§4.4), 그리고 특별히 주목할 만한 한 가지 설계 선택,
즉 연주자가 시행하는 제시부 반복을 도구가 처리하는 방식 (§4.5) 을
다룬다.

![**Figure 2.** System overview at startup, showing the title banner, the section-level timeline (top: pastel-blue exposition, pastel-amber development, pastel-green recapitulation), the now-playing analysis pane (theme zone, key, current measure), and the OSMD-rendered MusicXML score below.](figures/fig_a1_overview.png)
...
세 가지 표상과 `timeupdate` 이벤트에서의 갱신 동작은 표 3 에 정리되어 있다.
...
**표 3.** 세 가지 좌표 표상과 `timeupdate` 이벤트에서의 동작.
...
**표 4.** 섹션 단위 형식 라벨과 현재 위치 커서의 색상 인코딩 도식.
...
> K, codetta, retransition), and start/end measure ranges —
> deliberately mirror the analytic categories used in Allegraud et al.
> (2019) for the Mozart string-quartet sonata-form corpus. Because
> the field names map one-to-one and the measure-range conventions
> are shared, loading the 32-movement Algomus reference analyses into
> the tool requires only a thin ingestion adapter. We provide the
> schema and a worked example for K. 545; the adapter for the full
> Algomus corpus remains an open community contribution opportunity.
> The architectural point is that our learner-facing tool *amplifies*
> the classifier and annotation work of Allegraud et al. rather than
> competing with it: their corpus becomes loadable content for our
> visualization, and our visualization becomes a downstream
> evaluation surface for their classifier output.

본 스키마의 구조적 카테고리는 네 가지 섹션 유형 (exposition,
development, recapitulation, coda), 여섯 가지 주제 영역 유형 (P, T,
S, K, codetta, retransition), 그리고 시작/끝 마디 범위로 구성되며,
Allegraud et al. (2019) 이 모차르트 현악사중주 소나타 형식 코퍼스에
사용한 분석 카테고리를 의도적으로 미러링한다. 필드명이 1:1 로 대응
하고 마디 범위 컨벤션이 공유되므로, 32악장의 Algomus 참조 분석을
본 도구로 적재하는 데에는 얇은 ingestion 어댑터만으로 충분하다. 본
논문은 스키마와 K. 545 인스턴스를 공개하며, 전체 Algomus 코퍼스에
대한 어댑터는 커뮤니티 기여 기회로 남긴다. 본 학습자 친화 도구는
Allegraud et al. 의 분류기·주석 작업과 *경쟁* 하는 것이 아니라
그것을 *증폭* 한다는 것이 핵심이다: 그들의 코퍼스는 본 시각화의
로딩 가능 콘텐츠가 되고, 본 시각화는 그들의 분류기 출력에 대한
다운스트림 평가 표면이 된다.

## 4.4 Affordances for Active Listening / 능동 청취를 위한 어포던스

> Three interactive affordances are pedagogically central (Figure 3):

세 가지 인터랙티브 어포던스가 교수법적으로 중요하다 (Figure 3):
...
![**Figure 3.** Active-listening control panel and demo-mode fallback. The yellow *Demo mode* bar is automatically shown when no audio file is available. Below it: section-jump buttons (Exposition / Development / Recapitulation), playback-speed slider, color/cursor/loop toggles, calibration controls, and a keyboard-shortcut legend.](figures/fig_a2_timeline.png)
...
Carnegie Hall (2015) 녹음을 포함하여 K. 545 정전 디스코그래피의
대부분이 그렇다 — 오디오는 발전부에 도달하기 전에 제시부를 *두 번*
통과한다. 첫 번째 통과는 약 0:00 – 0:55, 두 번째 통과는 약 0:55 –
1:48, 그리고 1:48 이후에야 발전부가 시작된다. 두 가지 설계 대응이
가능하다. 단순한 대응은 반복을 무시하는 것이다 — 이 경우 커서는 오디오
시간을 따라 꾸준히 전진하고 분석 패널은 청자가 사실은 두 번째 제시부를
듣고 있는데도 "발전부" 를 표시하게 되어 교수법적으로 오도적이다. 본
연구가 채택하는 대응은 반복을 *폴딩 (folding)* 하는 것이다: 오디오가
두 번째 제시부에 진입하면 커서는 1마디로 돌아가 두 번째 청취와 동기
상태로 28마디까지 다시 걸어간다. 오디오가 두 번째 제시부의 끝을
지나야 비로소 커서가 발전부로 진입한다. 이로써 커서와 청자가 재생
전체에 걸쳐 일치 상태를 유지한다.
...
반복 경계 (두 번째 제시부가 시작·종료되는 오디오 시점) 는 키보드
단축키 (M, [, ]) 로 실제 녹음에 대해 1분 이내에 탭 보정 가능하며,
보정값은 브라우저 `localStorage` 에 영구 저장된다. 동일한 폴딩
동작이 표기된 제시부 반복이 있는 *모든 소나타 형식 작품* 에 변경
없이 이식된다.
...
> of this kind. Sonic Visualiser and Verovio Humdrum Viewer treat
> the score as static and the audio as a separate stream; Dezrann
> places annotation labels alongside the audio but does not fold
> them; the closest analogue in the MIR literature, Müller (2015,
> ch. 3), addresses repeats by inserting multiple parallel score
> branches, which preserves analytical correctness at the cost of
> multiple parallel score views. Our single-score-axis fold is
> consequently simpler both to implement and to visualize for a
> learner.

저자가 알기로, 어떤 기존 오픈 음악 시각화 도구도 연주자 선택
반복을 이러한 명시적 *시간 폴딩* 동작으로 처리하지 않는다. Sonic
Visualiser 와 Verovio Humdrum Viewer 는 악보를 정적인 것으로,
오디오를 분리된 스트림으로 다루며, Dezrann 은 오디오를 따라 주석
라벨을 배치하지만 그것을 폴딩하지 않는다. MIR 문헌에서 가장 가까운
대응인 Müller (2015, ch. 3) 의 작업은 다중 병렬 악보 분기 삽입을
통해 반복을 처리하는데, 이는 분석적 정확성을 보존하지만 다중의 병렬
악보 뷰를 대가로 치른다. 단일 score-time 축을 유지하는 본 폴딩
접근은 따라서 구현과 학습자 대상 시각화 모두에서 더 단순하다.
...
> RCT. The SUS items were administered following Brooke's (1996)
> original wording. The survey was self-administered, with the
> expert working through the live deployment at
> https://rosyrosys.github.io/sonata-form-viz/ in their own
> environment. Written responses were analysed by the author
> following the thematic-analysis approach of Braun and Clarke
> (2006), and the expert gave written informed consent for
...
원문에 따라 시행되었다. 설문은 자가 기입으로 진행되었고 전문가는
https://rosyrosys.github.io/sonata-form-viz/ 의 라이브 배포를
자신의 환경에서 사용하였다. 작성된 응답은 Braun & Clarke (2006)
의 주제 분석 접근법에 따라 저자가 분석하였으며, 전문가는 익명
보고에 대한 서면 동의를 제공하였다. 본 연구는 인간 대상 연구가
아닌 전문가 평가로 처리되었으며, 따라서 해당 기관의 정식 IRB 승인은
요구되지 않았다.

## 6.2 SUS result / SUS 결과

**Table 5 / 표 5.** Single-expert SUS response.
...
> them.*" In an optional open-comment field the expert added,
> unprompted, "*Personally, I hope this program is expanded and
> turned into a real, deployable tool.*" The two statements signal
> a single judgement: the tool's current single-work form is a
> demonstration artefact, and the expert's field practice would
> benefit from its maturation into a multi-work, deployment-grade
> resource. This motivates both the multi-piece corpus expansion
> listed as future work and a parallel commitment to
> production-readiness (documentation, classroom packaging,
> teacher-facing materials).

C5 (한 작품 깊이 vs 다곡 확장) 에 대해 전문가는 "다양한 작품이
있다면 적극 활용하겠다" 라고 응답하였고, 설문 말미의 자유 의견란
(Section D) 에서 비요청 코멘트로 "개인적으로 이 프로그램이
확장되어서 실제 상용화 도구로 나오면 좋겠다" 를 추가하였다. 두
진술은 단일 판단을 신호한다: 도구의 현재 단일 작품 형태는 *시연
작품* 이며, 전문가의 현장 실천은 도구가 다곡·배포 가능 등급의 자원
으로 성숙해질 때 이익을 본다. 이는 향후 작업으로 제시된 다곡 코퍼스
확장과 함께, *프로덕션 준비* (문서화·교실 패키징·교사용 자료)
라는 평행 약속을 동기화한다.

## 6.4 Implications for the RCT and tool roadmap / RCT 및 도구 로드맵 함의

> Four concrete revisions follow:
...
> 4. **Production-readiness track in parallel** (Section D comment):
>    documentation, classroom packaging, teacher-facing materials
>    for a v1.0 deployable release alongside the research RCT.
...
4. **프로덕션 준비 트랙 병행** (Section D 의견): 연구 RCT 와 나란히
   문서화·교실 패키징·교사용 자료를 갖춘 v1.0 배포 가능 릴리스
   준비.
