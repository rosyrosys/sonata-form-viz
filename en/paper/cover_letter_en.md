...
Re: Manuscript submission — "Teaching Sonata Form Through
Synchronized Visualization: Mozart's K. 545"
...
visualization that synchronizes a MusicXML score, audio playback,
and a two-level formal-analysis schema on a single timeline,
demonstrated on the first movement of Mozart's Piano Sonata in C
major, K. 545. The technical core is an OpenSheetMusicDisplay-based
pipeline that paints measure-level color overlays directly on the
rendered score, together with an exposition-repeat-folding map that
preserves analytical monotonicity under performer-elected repeats.
The analytical case centers on K. 545's subdominant recapitulation,
where the synchronized view makes the I→V / IV→I functional
symmetry visually self-evident.

The submission extends two lines of work previously published in
this journal: Allegraud et al. (2019) on automated sonata-form
structure learning, with which our JSON schema is intentionally
interoperable, and Ballester et al. (2025) on the Dezrann annotation
platform, to which our score-painted overlay is a complementary
learner-facing instrument.
...
A live deployment with a bundled Public Domain reference recording
is available at https://rosyrosys.github.io/sonata-form-viz/.

The manuscript does not include human-subjects evaluation; empirical
validation is deferred to a planned randomized controlled trial
under future IRB approval. The present submission has not been
submitted elsewhere, and there are no competing interests.
